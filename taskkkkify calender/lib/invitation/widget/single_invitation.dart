...
provider.categories.contains(invitationModel.category)?   '' : await FirebaseFirestore.instance.collection('users1')
  ...
  'categories': FieldValue.arrayUnion([invitationModel.category])
                   ...
                   await FirebaseFirestore.instance.collection('List').doc(invitationModel.listId).set({
                     'UID': FieldValue.arrayUnion([FirebaseAuth.instance.currentUser!.email])
                   }, SetOptions(merge: true));

                   var res = await FirebaseFirestore.instance.collection('tasks').where('ListName', isEqualTo: invitationModel.list).get();
                   for(int i = 0; i<res.docs.length; i++){
                     await FirebaseFirestore.instance.collection('tasks').doc(res.docs[i].id).set({
                       'UID': FieldValue.arrayUnion([FirebaseAuth.instance.currentUser!.email])
                     }, SetOptions(merge: true));
                   }

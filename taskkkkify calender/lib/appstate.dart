...
class AppState extends ChangeNotifier{
  ...
  getList(cat)async{
    ...
    final res = await FirebaseFirestore.instance.collection('List').where('CategoryName', isEqualTo: cat).where('UID', arrayContains: FirebaseAuth.instance.currentUser!.email).get();
 ...
 //   tasks.clear();
    ...
    final res = await FirebaseFirestore.instance.collection('tasks').where('CategoryName', isEqualTo: cat).where('ListName', isEqualTo: list).where('UID', arrayContains: FirebaseAuth.instance.currentUser!.email).where('status', isEqualTo: 'pending').get();
    for(int i = 0; i< res.docs.length; i++){
  ...
  getCompletedTasks(cat, list)async{
    ...
    final res = await FirebaseFirestore.instance.collection('tasks').where('CategoryName', isEqualTo: cat).where('ListName', isEqualTo: list).where('UID', arrayContains: FirebaseAuth.instance.currentUser!.email).where('status', isEqualTo: 'completed').get();
    for(int i = 0; i< res.docs.length; i++){
      Tasksss taskss = Tasksss(id: res.docs[i].id,
          task: res.docs[i]['Task'],
          priority: res.docs[i]['Priority'],
...
getListForTask()async{
  ...
  notifyListeners();
  final res = await FirebaseFirestore.instance.collection('List').where('UID', arrayContains: FirebaseAuth.instance.currentUser!.email).get();
  for(int i =0; i< res.docs.length; i++){
    TaskListModel task = TaskListModel(
      docId: res.docs[i].id,
      email: res.docs[i]['UID'],
...
bool disableDropDown = true;
 ...
 List<Tasksss> allTasks = [];
  ...
  getTasksofDate()async{
    ...
    //allTasks.clear();
    notifyListeners();
  var res =  await FirebaseFirestore.instance.collection('tasks').where('UID', arrayContains: FirebaseAuth.instance.currentUser!.email).get();
    for(int i = 0; i< res.docs.length; i++){
    ...
    print(date1);
      ...
      Tasksss tasks = Tasksss(id: res.docs[i].id,
          task: res.docs[i]['Task'],
          priority: res.docs[i]['Priority'],
          category: res.docs[i]['CategoryName'],
          list: res.docs[i]['ListName'],
...
print(allTasks.length);
  ...
  filterTasksByDate(date){
    ...
    filteredTasks.clear();
    notifyListeners();
    filteredTasks =  TasksModel.tasks.where((element) => element.deadline == date).toList();
    notifyListeners();
  ...
  getColorofDate(date){
    ...
    var date1 =  DateFormat("yyyy-MM-dd").format(date);
    checkingColor.clear();
    checkingColor = allTasks.where((element) => element.deadline == date1 && element.priority == 'High').toList();
    if(checkingColor.isNotEmpty){
      return 'High';
    }else{
      checkingColor = allTasks.where((element) => element.deadline == date1 && element.priority == 'Medium').toList();
      if(checkingColor.isNotEmpty){
        return 'Medium';
      }else{
        return 'Low';
